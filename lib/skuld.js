...
Consensus.prototype.add = function (norn) {
  ...
  return (this._norns[this._norns.length] = {
    ...
    propose : function (skuld, prepare) {
      consensus.propose(consensus._norns[this.id], skuld, prepare);
    },
    get wyrd() {
...
Consensus.prototype.propose = function (leader, skuld, prepare) {
  console.log(leader.name, "proposing", skuld);
  prepare(leader, skuld);
}
